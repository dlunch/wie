...
impl Image {
    ...
    async fn init(_: &mut Jvm, _: &mut WIPIJavaContxt, this: JvmClassInstanceHandle<Image>) -> JavaResult<()> {
        tracing::debug!("org.kwis.msp.lcdui.Image::<init>({:?})", &this);

        Ok(())
    }

    async fn create_image(jvm: &mut Jvm, _: &mut WIPIJavaContxt, width: i32, height: i32) -> JavaResult<JvmClassInstanceHandle<Image>> {
    ...
    async fn get_graphics(jvm: &mut Jvm, _: &mut WIPIJavaContxt, this: JvmClassInstanceHandle<Self>) -> JavaResult<JvmClassInstanceHandle<Graphics>> {

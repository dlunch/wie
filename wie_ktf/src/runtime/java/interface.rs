...
pub async fn java_class_load(core: &mut ArmCore, system: &mut SystemHandle, ptr_target: u32, name: String) -> anyhow::Result<u32> {
    ...
    let class = if name.as_bytes()[0] == b'[' {
        KtfJvm::new(core, system).load_array_class(&name).await?.map(|x| x.class)
    } else {
        KtfJvm::new(core, system).load_class(&name).await?
    };
...
pub async fn java_throw(_: &mut ArmCore, _: &mut SystemHandle, error: String, a1: u32) -> anyhow::Result<u32> {
...
async fn register_java_string(core: &mut ArmCore, system: &mut SystemHandle, offset: u32, length: u32) -> anyhow::Result<u32> {
    ...
    let instance = JavaString::from_utf16(&mut jvm.jvm(), bytes_u16).await?;

    Ok(jvm.class_raw(&instance) as _)
}
...
async fn jb_unk4(_: &mut ArmCore, _: &mut SystemHandle, a0: u32, a1: u32) -> anyhow::Result<u32> {
    tracing::warn!("stub jb_unk4({:#x}, {:#x})", a0, a1);

    Ok(0)
}

async fn jb_unk5(_: &mut ArmCore, _: &mut SystemHandle, a0: u32, a1: u32) -> anyhow::Result<u32> {
    tracing::warn!("stub jb_unk5({:#x}, {:#x})", a0, a1);

    Ok(0)
}

async fn jb_unk7(_: &mut ArmCore, _: &mut SystemHandle, a0: u32) -> anyhow::Result<u32> {
    tracing::warn!("stub jb_unk7({:#x})", a0);

    Ok(0)
}

async fn jb_unk8(_: &mut ArmCore, _: &mut SystemHandle, a0: u32) -> anyhow::Result<u32> {
...
pub async fn java_new(core: &mut ArmCore, system: &mut SystemHandle, ptr_class: u32) -> anyhow::Result<u32> {
    ...
    let raw = jvm.class_raw(&instance);
...
pub async fn java_check_cast(_: &mut ArmCore, _: &mut SystemHandle, ptr_class: u32, ptr_instance: u32) -> anyhow::Result<u32> {

...
impl StringBuffer {
    ...
    async fn init(context: &mut dyn JavaContext, this: JavaObjectProxy) -> JavaResult<()> {
        log::trace!("java.lang.StringBuffer::<init>({:#x})", this.ptr_instance);

        let java_value_array = context.instantiate_array("C", 16)?;
        context.put_field(&this, "value", java_value_array.ptr_instance)?;
        context.put_field(&this, "count", 0)?;

        Ok(())
    }

    async fn append_string(context: &mut dyn JavaContext, this: JavaObjectProxy, string: JavaObjectProxy) -> JavaResult<JavaObjectProxy> {
        log::trace!(
            "stub java.lang.StringBuffer::append({:#x}, {:#x})",
            this.ptr_instance,
            string.ptr_instance
        );
        let current_count = context.get_field(&this, "count")?;
        ...
        let java_value_aray = JavaObjectProxy::new(context.get_field(&this, "value")?);
        context.store_array(&java_value_aray, current_count, &value_to_add)?;

        Ok(this)
    }

    async fn to_string(context: &mut dyn JavaContext, this: JavaObjectProxy) -> JavaResult<JavaObjectProxy> {
        log::trace!("java.lang.StringBuffer::toString({:#x})", this.ptr_instance);

        let java_value = JavaObjectProxy::new(context.get_field(&this, "value")?);
        let count = context.get_field(&this, "count")?;

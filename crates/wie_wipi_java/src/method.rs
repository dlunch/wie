...
pub trait MethodBody<E> {
    fn call(&self, context: &mut dyn JavaContext, args: &[u32]) -> Result<u32, E>;
}
...
where
    F: Fn(&mut dyn JavaContext) -> Result<R, E>,
    R: TypeConverter<R>,
{
    fn call(&self, context: &mut dyn JavaContext, _: &[u32]) -> Result<u32, E> {
...
where
    F: Fn(&mut dyn JavaContext, P0) -> Result<R, E>,
    R: TypeConverter<R>,
    P0: TypeConverter<P0>,
{
    fn call(&self, context: &mut dyn JavaContext, args: &[u32]) -> Result<u32, E> {
...
where
    F: Fn(&mut dyn JavaContext, P0, P1) -> Result<R, E>,
    R: TypeConverter<R>,
    P0: TypeConverter<P0>,
    P1: TypeConverter<P1>,
{
...
where
    F: Fn(&mut dyn JavaContext, P0, P1, P2) -> Result<R, E>,
    R: TypeConverter<R>,
    P0: TypeConverter<P0>,
    P1: TypeConverter<P1>,
    P2: TypeConverter<P2>,
...
where
    F: Fn(&mut dyn JavaContext) -> Result<R, E> + 'static,
...
where
    F: Fn(&mut dyn JavaContext, P0) -> Result<R, E> + 'static,
    R: TypeConverter<R> + 'static,
...
where
    F: Fn(&mut dyn JavaContext, P0, P1) -> Result<R, E> + 'static,
    R: TypeConverter<R> + 'static,
    P0: TypeConverter<P0> + 'static,
...
where
    F: Fn(&mut dyn JavaContext, P0, P1, P2) -> Result<R, E> + 'static,
    R: TypeConverter<R> + 'static,
    P0: TypeConverter<P0> + 'static,
    P1: TypeConverter<P1> + 'static,

...
mod tests {
    ...
    fn thumb_countdown_has_a_backedge_and_conditional_fallthrough() {
        ...
        let Operation::Alu {
            op: AluOp::Move,
            destination: Some(decoded_destination),
            left: Value::Immediate(0),
            right:
                Operand {
        ...
        let Operation::Branch {
            target: BranchTarget::Address(decoded_target),
            link: None,
            exchange: false,
        } = branch.operation
        else {
        ...
        let Operation::Branch {
            target: BranchTarget::Register(decoded_target),
            link: None,
            exchange: true,
    ...
    fn arm_conditions_comparisons_and_backedges() {
        ...
        let Operation::Alu {
            op: AluOp::Add,
            destination: Some(decoded_destination),
            left: Value::Register(decoded_left),
            set_flags: false,
            ..
        } = ir.blocks[0].instructions[0].operation
        else {
    ...
    fn memory_width_sign_index_and_writeback_are_preserved() {
        ...
        let Operation::Load {
            ...
            address:
                MemoryOperand {
                    base: Value::Register(decoded_address_base),
                    offset:
                        Operand {
                    ...
                    subtract: false,
                    pre_index: false,
                    write_back: Some(decoded_address_write_back),
                },
            width: Width::Byte,
            signed: false,
        } = ir.blocks[0].instructions[0].operation
        ...
        let Operation::Store {
            value: Value::Register(decoded_value),
            width: Width::Word,
            address:
                MemoryOperand {
                    subtract: true,
        ...
        let Operation::Load {
            ...
            address:
                MemoryOperand {
                    offset:
                        Operand {
                            value: Value::Register(decoded_address_offset_value),
                            ..
                        },
                    write_back: None,
                    ..
    ...
    fn arm_unconditional_space_accepts_only_blx_and_valid_prefetch_hints() {
        for (opcode, target) in [(0xfa000000, 0x1009), (0xfb000000, 0x100b), (0xfaffffff, 0x1005)] {
            ...
            let Operation::Branch {
                target: BranchTarget::Address(actual_target),
                link: Some(decoded_link),
                exchange: true,
            } = ir.blocks[0].instructions[0].operation
            else {
            ...
            for (pre_index, write_back) in [(true, false), (true, true), (false, true)] {
                for subtract in [false, true] {
                    for (immediate, bits, value) in [(true, 0xa0b, Value::Immediate(0xab)), (false, 4, Value::Register(Reg::new(4)))] {
                        ...
                        let actual_write_back = actual_write_back.map(Reg::index);
    ...
    fn arm_data_processing_opcode_families_preserve_operands() {
        for (opcode, op) in [
            ...
            let Operation::Alu {
                ...
                right:
                    Operand {
                        value: Value::Register(decoded_right_value),
                        shift: Shift::Lsl,
                        amount: ShiftAmount::Immediate(0),
                    },
                set_flags: true,
